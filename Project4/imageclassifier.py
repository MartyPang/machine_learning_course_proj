...
# path = r'/home/martypang/PyCharmProjects/machine_learning_proj/data/faces_4/an2i/an2i_left_angry_open_4.pgm'
# im = Image.open(path)
# im.show()
...
for i in range(len(imgfilenames)):
    for j in range(len(imgfilenames[i])):
        ...
        reshape_img_array = image_array.reshape((28, 28, 1))
...
# create model
model = resnet_v2(input_shape, 29)
...
model.fit_generator(datagen.flow(np.asarray(X_train), np.asarray(y_train), batch_size=batch_size),
                    validation_data=(np.asarray(X_test), np.asarray(y_test)),
